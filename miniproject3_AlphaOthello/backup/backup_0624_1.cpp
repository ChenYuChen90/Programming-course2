...
public:
    ...
    // minimax recursion ()
    // cur board , next choice dics position on cur board , search depth , opponent or me , white or black 
    int minimax(array<array<int, SIZE>, SIZE> now_board, Point point, int depth, bool player_type, int cur_player){
        if(depth == 0){
            return evaluation(point);
        }
        OthelloBoard round(now_board, point, cur_player);
        // 若是搜尋到點無法放置時該怎麼辦 ? 
        if(!round.put_disc(point)){
            return 0;
        }
        if(player_type){
            int best_value = -INF_VALUE;
            for(auto p : round.get_next_valid_spots()){
                best_value = max(best_value, minimax(round.get_cur_board(), p, depth - 1, false, round.get_cur_player()));
            }
            return best_value;
        } else {
            int best_value = INF_VALUE;
            for(auto p : round.get_next_valid_spots()){
                best_value = min(best_value, minimax(round.get_cur_board(), p, depth - 1, true, round.get_cur_player()));
            }
            return best_value;
        }
    }
    ...
    Point best_choice(){
        ...
        for(int i = 0; i < next_valid_spots.size(); i++){
            n_valid_spots_value[i] = minimax(board, next_valid_spots[i], 5, false, player);
